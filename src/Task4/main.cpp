...
int main(int argc, char*argv[])
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float timeValue = glfwGetTime();
		float trslWidth = (sin(timeValue));

		//f = abs(trslWidth);

		//设置变换矩阵
		glm::mat4 trans = glm::mat4(1.0f);
		trans = glm::scale(trans, glm::vec3(f, f, 1));
		glm::mat4 rot = glm::mat4(1.0f);
		rot = glm::rotate(rot, trslWidth * 3.14f, glm::vec3(0, 0, 1));
		glm::mat4 trsl = glm::mat4(1.0f);
		trsl = glm::translate(trsl, glm::vec3(trslWidth, -trslWidth, 0));
		rtshader->setMatrix("transform", trans);
		rtshader->setMatrix("rotTransform", rot);
		rtshader->setMatrix("translateform", trsl);
		glBindVertexArray(VAO[1]);
